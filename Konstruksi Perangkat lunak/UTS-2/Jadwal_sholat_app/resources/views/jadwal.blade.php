...
<div class="container my-5">
    ...
    <div class="card shadow">
        ...
        <div class="card-body text-center">
            @if(isset($ayat))
                <h5 class="mb-3">Surah {{ $ayat['surah']['englishName'] ?? 'Unknown Surah' }} (Verse {{ $ayat['surah']['number'] ?? '-' }})</h5>
                <p class="verse-text">{{ $ayat['text'] ?? '-' }}</p>
                <p class="verse-translation mt-3">{{ $terjemahan['data']['text'] ?? 'Translation not available' }}</p>
            @else
